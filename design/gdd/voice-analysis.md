...
| **Réseau** (5) | — | `VoiceFrame`, ses cinq champs ; **`r'`, un seul nombre, à la connexion et à chaque recalibration** — domaine : *Formulas* §5 | Client → hôte. `VoiceFrame` : ADR-0003:134, 224. `r'` : **routage en attente** — ADR-0003:337 transmet encore « la calibration » entière (*Formulas* §1, « Position, repos, et où chaque grandeur existe »). Le réseau décide de l'échantillonnage vers sa cadence (ADR-0003:224), pas Core ; `Tick` y sert à repérer les trous |
...
| `r'` | client, à chaque commit de profil | profil local, `Margin_dB` local | client ; hôte | envoyé à l'hôte **à la connexion et à chaque recalibration** (décision E5). **Routage en attente** : ADR-0003:337 transmet encore « la calibration » entière, et son amendement E5 n'est pas écrit. Jusque-là, la comparaison côté hôte n'a pas de donnée pour tourner |
...
| `p` — part des trames à `Loudness = 0` sur bruit ambiant (AC-40) | — | — | Mesure : système 1 ; part suffisante : système 11 | **EN ATTENTE** | B2 — étape de silence du prototype navigateur |
...
| AC-40 | **« Quand je me tais, rien ne bouge. »** GIVEN 30 s de bruit ambiant réel enregistré — clavier, ventilateur, respiration, conversation à côté — et un profil calibré **dans la même pièce** THEN `Loudness = 0` sur au moins **p %** des trames. `p` est **EN ATTENTE** : le prototype navigateur mesure la part réellement atteinte sur une étape de silence (B2), et le système 11 dit celle qui lui suffit — cela ne se fixe pas sur le papier | `[HUMAIN]` puis rejouable en `[UNIT]` |
