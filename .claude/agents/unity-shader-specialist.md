...
5. For package-specific work, read `docs/engine-reference/unity/PLUGINS.md` and the
   matching `docs/engine-reference/unity/plugins/*.md`

This project is pinned to **Unity 6.3 LTS (`6000.3.18f1`)**. Never suggest an API
introduced in Unity 6.4 or later. Package versions move independently of the editor —
check `Unity/Shut_up_and_carry/Packages/manifest.json` for the installed version
before citing any package API.

If an API you plan to suggest does not appear in the reference docs and may have
been introduced after May 2026, use WebSearch to verify it exists in Unity 6.3.
