...
## Introduced by Unity 6.3 itself
...
|---|---|---|
| `[SerializeField]` on properties/methods/types | `[field: SerializeField]` on auto-properties, or a plain private field | **Compile error** in 6.3, not a warning |
| `AccessibilityNode.selected` | `AccessibilityNode.invoked` | Old name still present; obsolete in a future release |
| `AdditionalBakedProbes` (removed) | `LightTransport.IProbeIntegrator` | Experimental API, fully removed |
| `CustomBake` (obsolete) | `LightTransport.IProbeIntegrator` | |
| Netcode for GameObjects 1.x | Netcode for GameObjects 2.x | 1.x deprecated wholesale |
| `NetworkTransform.Update` override | `NetworkTransform.OnUpdate` (non-authority) or `INetworkUpdateSystem` (authority) | No longer supported as an override point |
| URP Compatibility Mode code paths | RenderGraph | Stripped by default; `URP_COMPATIBILITY_MODE` is conversion-only, gone in 6.4+ |
| `PlayerSettings.Android.androidIsGame` | **App Category** Player setting | Affects Android 16+ large-screen behavior |
| Android round / legacy icons | Adaptive icons | |
| `UPM_NPM_CACHE_PATH` env var | `UPM_CACHE_ROOT` | |
| Legacy ETC compressor | Default compressor (auto-migrated) | Configure ETCPACK manually only if pixel-identical output is required |
| Magic Leap XR Plugin | — | Deprecated; new projects cannot target Magic Leap x86-64 |
| Advertisement Legacy | Ads Mediation (Unity LevelPlay) | |

**Type changes requiring assembly recompilation** (implicit conversions keep source
compiling, but precompiled `.dll`s break):

| Was | Now |
|---|---|
| `AccessibilityRole` / `AccessibilityState` backed by `int` | backed by `byte` |
| `AccessibilityRole` as a flags enum | standard enum — one role per node |
| `GlobalIllumination.*.instanceID` (`int`) | `entityID` (`EntityId`) |
| `Scene.handle` (`int`) | `SceneHandle` |
